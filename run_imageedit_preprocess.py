...
def annotate(cfg, segmenter, rotation, reconstructor, captioner, prompter, global_data, logger, device, class_index):
    ...
    all_sum = 0 # 总数
    correct_sum = 0 # 能通过最终检验的
    correct_before_spatialQA_total_sum = 0 # 能通过segment和reconstructor的
    ...
    for i, filepath in tqdm(enumerate(global_data), ncols=25):
        ...
        try:
            ...
            if class_index not in [20,21,22,23,24,25,26,27]: # 这类物体不要求朝向，因此在这个任务中任务就不需要获取朝向
            ...
            if len(detection_list) != 1:
                raise SkipImageException("detection_list is not 1")


            if class_index in [20,21,22,23,24,25,26,27]:
                npy_to_save = np.zeros((len(detection_list),5))
                for i in range(len(detection_list)):
                    pos = detection_list[i]['pcd'].get_center()
                    npy_to_save[i][0] = -pos[0]
                    npy_to_save[i][1] = -pos[1]
                    npy_to_save[i][2] = pos[2]
                    npy_to_save[i][3] = detection_list[i]['left_edge']
                    npy_to_save[i][4] = detection_list[i]['right_edge']
                np.save(preprocess_path, npy_to_save)
                
            elif class_index in [28]:
                npy_to_save = np.zeros(len(detection_list))
                for i in range(len(detection_list)):
                    A_rotation_matrix = detection_list[i]["rotation_matrix"]
                    theta_A = np.arctan2(A_rotation_matrix.T[0][2], A_rotation_matrix.T[0][0])
                    A_center = detection_list[i]["pcd"].get_center()
                    R = detection_list[i]["pcd"].get_rotation_matrix_from_xyz((0, 0, theta_A)) 
                    detection_list[i]["pcd"] = detection_list[i]["pcd"].rotate(R)
                    A_length = detection_list[i]["pcd"].get_axis_aligned_bounding_box().get_extent()[0]
                    A_height = detection_list[i]["pcd"].get_axis_aligned_bounding_box().get_extent()[1]
                    A_width = detection_list[i]["pcd"].get_axis_aligned_bounding_box().get_extent()[2]
                    npy_to_save[i] = A_length * A_height * A_width
                np.save(preprocess_path, npy_to_save)       
    
            elif class_index in [29]:
                npy_to_save = np.zeros(len(detection_list))
                for i in range(len(detection_list)):
                    npy_to_save[i] = detection_list[i]["pcd"].get_axis_aligned_bounding_box().get_extent()[1]
                np.save(preprocess_path, npy_to_save)
                
            elif class_index in [30]:
                npy_to_save = np.zeros(len(detection_list))
                for i in range(len(detection_list)):
                    A_rotation_matrix = detection_list[i]["rotation_matrix"]
                    theta_A = np.arctan2(A_rotation_matrix.T[0][2], A_rotation_matrix.T[0][0])
                    ...
                    detection_list[i]["pcd"] = detection_list[i]["pcd"].rotate(R)
                    A_length = detection_list[i]["pcd"].get_axis_aligned_bounding_box().get_extent()[0]
                    npy_to_save[i] = A_length
                np.save(preprocess_path, npy_to_save)
                
            elif class_index in [31]:
                npy_to_save = np.zeros(len(detection_list))
                for i in range(len(detection_list)):
                    A_rotation_matrix = detection_list[i]["rotation_matrix"]
                    theta_A = np.arctan2(A_rotation_matrix.T[0][2], A_rotation_matrix.T[0][0])
                    ...
                    detection_list[i]["pcd"] = detection_list[i]["pcd"].rotate(R)
                    A_width = detection_list[i]["pcd"].get_axis_aligned_bounding_box().get_extent()[2]
                    npy_to_save[i] = A_width
                np.save(preprocess_path, npy_to_save)

                
            # if class_index in [10,11,12,13]:
            #     npy_to_save = np.zeros(len(detection_list))
            #     for i in range(len(detection_list)):
            #         npy_to_save[i] = detection_list[i]['square']
            #     np.save(preprocess_path, npy_to_save)
            # elif class_index in [23,24,25,26,31,32,33,34,41,42]:
            #     npy_to_save = np.zeros((len(detection_list),5))
            #     for i in range(len(detection_list)):
            #         pos = detection_list[i]['pcd'].get_center()
            #         npy_to_save[i][0] = -pos[0]
            ...
            #         npy_to_save[i][4] = detection_list[i]['right_edge']
            #     np.save(preprocess_path, npy_to_save)
            # elif class_index in [35,36,37,38,39,40]:
            #     npy_to_save = np.zeros((len(detection_list),3,3))
            #     for i in range(len(detection_list)):
            #         npy_to_save[i] = detection_list[i]['rotation_matrix']
            #     np.save(preprocess_path, npy_to_save)
    ...
    if correct_before_spatialQA_total_sum > 0:
        ...
        # 把acc写到txt文件中
        with open(f'acc_{postfix}.txt', 'a') as f:
    ...
    # 把gt_list写到txt中，每行一个
    with open(f'gt_list_{postfix}.txt', 'a') as f:
